...
use Karthus\Contract\Able\Macroable;
...
use Karthus\Exception\EncodingException;
use Karthus\Exception\FileException;
use Karthus\Exception\InvalidResponseException;
...
class Response implements PsrResponseInterface, ResponseInterface, Sendable {
    ...
    public function redirect(
        ...
        $toUrl = value(function () use ($toUrl, $schema) {
            if (! ApplicationContext::hasContainer() || Strings::startsWith($toUrl, ['http://', 'https://'])) {
                return $toUrl;
            }
            ...
            return $schema . '://' . $host . (Strings::startsWith($toUrl, '/') ? $toUrl : '/' . $toUrl);
        });
