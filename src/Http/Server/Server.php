...
class Server {
    ...
    public function start() {
        ...
        foreach ($this->callbacks as $pattern => $callback) {
            $server->handle($pattern, function (Request $request, Response $response) use ($callback) {
                try {
                    // 生成PSR的request,response
                    $request  = (new ServerRequestFactory)->createServerRequestFromSwoole($request);
                    $response = (new ResponseFactory)->createResponseFromSwoole($response);
                    // 执行回调
